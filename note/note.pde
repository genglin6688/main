...
void draw() {
  ...
  if (page.equals("noteLists")) {
    ...
    if (searchText.equals("")) {
      ...
      for (int i=0+section*5; i<note.size()&&i<5+section*5; i++) {
        ...
        String tempText=note.get(i).text;

        if (tempText.length()>30) {

          tempText=tempText.substring(0, 30);
        ...
        text(tempText, width/2+175, 325+(i%5)*110);
      }
      ...
      for (int i=0+section*5; i<searchnote.size()&&i<5+section*5; i++) {
        ...
        String tempText=searchnote.get(i).text;
  ...
  if (page.equals("settings") ) {
    ...
    if (importantNoteToggleB==255&&importantNoteToggleG==255&&importantNoteToggleR==255) {
      ...
      text("warning, your selected color is the same with none-important notes", width/2, height/2+275);
    }
  ...
  if (page.equals("login")) {
    String tempText[]=loginCode.split("");
    textSize(30);
    textAlign(CENTER, CENTER);
    fill(0);
    text("enter log in code", width/2, height/2-300);
    fill(255);
    rect(width/2-200, height/2-50, 400, 100);
    for (int i=0; i<4; i++) {
      fill(255);
      rect(width/2-140+70*i, height/2-30, 60, 60);
      if (loginCode.length()>i) {
        fill(0);
        text(tempText[i], width/2-110+70*i, height/2);
      }
    }
  }
}
...
void keyPressed() {
  ...
  if (page.equals("login")) {
    ...
    if (loginCode.length()<4) {
      ...
      if(key==BACKSPACE){
       loginCode=loginCode.substring(0,loginCode.length()-1);  
      ...
      if (key<=126&&key>=32||key==BACKSPACE||key==ENTER) {
        ...
        for (int i=0; i<note.size(); i++) {
          ...
          if (noteAdded==false) {
            ...
            if (str(note.get(i).noteNumber).indexOf(searchText)!=-1) {
...
void keyReleased() {
  ...
  /**
   * unlock animations
   * @param time the time when the program gets unlocked
   */
