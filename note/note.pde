...
void draw() {
  ...
  if (page.equals("noteGraphs")) {
    ...
    save=new String[note.size()]; 
    for (int i=0; i<note.size(); i++) {
      save[i]= note.get(i).title+"/t"+note.get(i).text+"/t"+str(note.get(i).x)+"/t"+str(note.get(i).y)+"/t"+str(note.get(i).noteNumber);
    }
  ...
  if (page.equals("noteLists")) {
    ...
    fill(0);
    text("sort by bookmark", width/2, 225);
    imageMode(CENTER);
    ...
    if (searchHighlight) {
      if (key<=126&&key>=32||key==BACKSPACE||key==ENTER) {
        ...
        for (int i=0; i<note.size(); i++) {
          ...
          searchnote.set(i, note.get(i));
        }
        for ( int i=note.size()-1; i<searchnote.size(); i++) {
          searchnote.remove(i);
        }
        for (int i=0; i<searchnote.size(); i++) {
          for (int n=0; n<searchnote.size(); n++) {
            if (searchnote.get(i).noteNumber==searchnote.get(n).noteNumber) {
              if (i!=n) {
                searchnote.remove(n);
              }
            }
        ...
        for (int i=0; i<searchnote.size(); i++) {
          if (searchnote.get(i).title.indexOf(searchText)==-1) {
            searchnote.remove(i);
          }
        }
      }
